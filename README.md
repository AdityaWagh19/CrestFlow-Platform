...
> **Implementation Status:** Plan 00 (Monorepo + Tooling) complete. Plan 01 (Auth + Turnkey Wallet) complete. Plans 02–11 in progress.

### Foundation (build these first — everything else depends on them)

| Plan | What it does |
|---|---|
| [Plan 01 — Auth + Wallet](./plans/01-auth-turnkey-onboarding.md) | Signs users in via Google OAuth. Creates an embedded Algorand wallet per user using Turnkey TEE — no private keys ever touch CrestFlow servers. Implemented |
...
|---|---|---|
| Embedded Wallet | Turnkey TEE | P0 — implemented |
| Lending | Folks Finance | P0 — in progress |
| DEX / Swap | Haystack Router (Tinyman + Pact aggregation) | P0 — in progress |
| DEX / LP | Tinyman V2 | P0 — in progress |
| DEX / LP | Pact | P0 — in progress |
| KYC | Veriff (doc + liveness + AML) | P1 — planned |
| Decentralised Identity | GoPlausible (DID + VC) | P1 — planned |
| Fiat On-Ramp | Transak (INR → USDC) | P1 — planned |
| Fiat Off-Ramp | Transak (USDC → INR) | P1 — planned |
| x402 Payments | Goplusfable Facilitator | P1 — planned |
| AI — Primary | GPT-4.1-mini (OpenAI) | P0 — in progress |
| AI — Fallback | Gemini 3.5 Flash | P0 — in progress |
| Market Data | CoinGecko | P0 — in progress |
| On-chain Data | Algorand Indexer | P0 — in progress |
...
├── docker-compose.yml # PostgreSQL + Redis local dev environment
├── turbo.json # Turborepo pipeline config
├── pnpm-workspace.yaml
├── apps/
│ ├── copilot-api/ # Scaffolded — main backend (Node.js + Express)
│ │ ├── src/
│ │ │ ├── modules/
│ │ │ │ ├── identity/ # Plan 01 — Auth, Google OAuth, Turnkey wallet
│ │ │ │ ├── knowledge/ # Plan 02 — data adapters (in progress)
│ │ │ │ ├── portfolio/ # Plan 03 — Engine 1 (in progress)
│ │ │ │ ├── risk/ # Plan 04 — Engine 2 (in progress)
│ │ │ │ ├── strategy/ # Plan 05 — Engine 3 (in progress)
│ │ │ │ ├── yield/ # Plan 06 — Engine 4 (in progress)
│ │ │ │ ├── intelligence/ # Plan 07 — Engine 5 + Copilot (planned)
│ │ │ │ ├── execution/ # Plan 08 — Engine 6 (planned)
│ │ │ │ ├── audit/ # Plan 09 — Audit layer (planned)
│ │ │ │ └── kyc/ # Plan 10 — KYC + DID + on-ramp (planned)
│ │ │ ├── middleware/ # authenticate, rate-limit, x402, error-handler
│ │ │ ├── queues/ # BullMQ workers (stub) for all 6 engines
│ │ │ ├── lib/ # redis, algorand, jwt, bullmq, openai singletons
...
│ │ └── Dockerfile
│ └── web/ # Frontend — Vite + React (planned)
├── packages/
│ ├── shared/ # Scaffolded — shared types, decimal helpers
│ └── contracts/ # Scaffolded — Algorand Puya Python contracts
├── project-context/
...
├── plans/
│ ├── 00-monorepo-tooling-setup.md # complete
│ ├── 01-auth-turnkey-onboarding.md # complete
│ ├── 02-financial-knowledge-layer.md # in progress
...
├── architecture_review.md # Architecture review by Staff Architect
└── architecture_audit_v2.md # Pre-implementation architecture audit v2
```
